...
namespace HOOKS
{
	...
	void __stdcall HookedPaintTraverse(int VGUIPanel, bool ForceRepaint, bool AllowForce)
	{
		...
		if (panel_name == "FocusOverlayPanel")
		{
			...
			RENDER::DrawF(w - 167 + 7, 5, FONTS::font_watermark, false, false, CColor(255, 255, 255, 255), "1tapgang |");
	...
	void getconfig()
	{
		...
		szPath1 = std::string(path) + "\\1tapgang\\*";
	...
	long __stdcall Hooked_EndScene(IDirect3DDevice9* pDevice)
	{
		...
		if (SETTINGS::settings.menu_open)
		{
			...
			ImGui::Begin("1tapgang fixed by carbon#1337", &menu_open, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoTitleBar);
			...
			ImGui::Text("1tapgang fixed by carbon#1337");
